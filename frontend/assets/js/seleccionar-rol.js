...
/* =========================================================
   CLARO ATENCIÓN 360 - SELECCIÓN DE ROL
   Pantalla informativa previa al login
========================================================= */
...
const RoleData = {
  ...
  "cliente-empresa": {
    ...
    label: "Empresas",
    description:
      "Acceso para representantes de empresas que gestionan tickets, incidencias, servicios empresariales y seguimiento de SLA.",
    permissions: [
      "Reportar tickets empresariales",
      "Consultar SLA",
      "Gestionar servicios corporativos",
      "Ver historial empresarial"
    ],
...
function removeObsoleteDemoButton() {
  $("#loadDemoBtn")?.remove();
}
...
function bindRecommendation() {
  $("#recommendRoleBtn")?.addEventListener("click", () => {
    ...
    showToast({
      ...
      message: `Perfil sugerido: ${data.title}.`,
      type: "success"
    });
  });
}
...
function generateBotResponse(prompt) {
  ...
  if (text.includes("reclamo") || text.includes("personal")) {
    selectRole("cliente-persona", false);
    return "Para registrar o consultar reclamos personales, corresponde Cliente Persona.";
  }

  if (text.includes("empresa") || text.includes("corporativo") || text.includes("ticket")) {
    selectRole("cliente-empresa", false);
    return "Para gestionar servicios corporativos, tickets empresariales o SLA de empresa, corresponde Cliente Empresa.";
  }

  if (text.includes("atiendo") || text.includes("atender") || text.includes("casos asignados")) {
    selectRole("asesor", false);
    return "Si atiendes casos asignados y registras avances, corresponde Asesor de Atención.";
  }

  if (text.includes("sla") || text.includes("supervisar") || text.includes("asignar")) {
    selectRole("supervisor", false);
    return "Si necesitas clasificar, asignar o controlar SLA, corresponde Supervisor.";
  }

  if (text.includes("admin") || text.includes("usuarios") || text.includes("roles") || text.includes("catálogos")) {
    selectRole("admin", false);
    return "Para configurar usuarios, roles, catálogos o auditoría, corresponde Administrador.";
  }

  return "Puedo ayudarte a elegir el perfil correcto. Indica si eres cliente, empresa, asesor, supervisor o administrador.";
}
